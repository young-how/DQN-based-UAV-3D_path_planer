...
#VA网络(4层)，用于Dueling DQN算法(用于FANET训练环境)
class VAnet4(torch.nn.Module):
    def __init__(self, param):
        super(VAnet4, self).__init__()
        state_dim=int(param.get('w'))
        hidden_dim=int(param.get('hiden_dim'))
        action_dim=int(param.get('output'))
        self.fc1 = torch.nn.Linear(state_dim, 2*hidden_dim)  # 共享网络部分
        self.fc2 = torch.nn.Linear(2*hidden_dim, hidden_dim)  # 共享网络部分
        self.fc3 = torch.nn.Linear(hidden_dim, hidden_dim)  # 共享网络部分
        self.fc_A = torch.nn.Linear(hidden_dim, action_dim)
        self.fc_V = torch.nn.Linear(hidden_dim, 1)
...
#VA网络(5层)，用于Dueling DQN算法(用于FANET训练环境)
class VAnet5(torch.nn.Module):
    def __init__(self, param):
        super(VAnet5, self).__init__()
        state_dim=int(param.get('w'))
        hidden_dim=int(param.get('hiden_dim'))
        action_dim=int(param.get('output'))
        self.fc1 = torch.nn.Linear(state_dim, 2*hidden_dim)  # 共享网络部分
        self.fc2 = torch.nn.Linear(2*hidden_dim, hidden_dim)  # 共享网络部分
        self.fc3 = torch.nn.Linear(hidden_dim, hidden_dim)  # 共享网络部分
        self.fc4 = torch.nn.Linear(hidden_dim, hidden_dim)  # 共享网络部分
        self.fc_A = torch.nn.Linear(hidden_dim, action_dim)
        self.fc_V = torch.nn.Linear(hidden_dim, 1)
...
#SAC算法所需网络2（离散动作空间）
class PolicyNet_SAC2(torch.nn.Module):
    def __init__(self, param:dict):
        super(PolicyNet_SAC2, self).__init__()
        state_dim=int(param.get('w'))
        hidden_dim=int(param.get('hiden_dim'))
        self.action_dim=int(param.get('output'))
        self.fc1 = torch.nn.Linear(state_dim, hidden_dim)
        ...
        self.fc4 = torch.nn.Linear(hidden_dim, self.action_dim)
...
class QValueNet_SAC2(torch.nn.Module):
    ''' 只有一层隐藏层的Q网络 '''
    def __init__(self, param:dict):
        super(QValueNet_SAC2, self).__init__()
        state_dim=int(param.get('w'))
        hidden_dim=int(param.get('hiden_dim'))
        action_dim=int(param.get('output'))
        self.fc1 = torch.nn.Linear(state_dim, hidden_dim)
...
#SAC算法所需网络（离散动作空间）
class PolicyNet_SAC3(torch.nn.Module):
    def __init__(self, param:dict):
        super(PolicyNet_SAC3, self).__init__()
        state_dim=int(param.get('w'))
        hidden_dim=int(param.get('hiden_dim'))
        self.action_dim=int(param.get('output'))
        self.fc1 = torch.nn.Linear(state_dim, hidden_dim)
        ...
        self.fc5 = torch.nn.Linear(hidden_dim, self.action_dim)
    ...
    def forward(self, x):
        ...
        x6=self.fc5(x5)
        if x6.shape==torch.Size([self.action_dim]):
            x7=F.softmax(x6, dim=0)
            if math.isnan(x7.tolist()[0]):
                print('err')
                return x7
            else: 
                return x7
...
class QValueNet_SAC3(torch.nn.Module):
    ''' 只有一层隐藏层的Q网络 '''
    def __init__(self, param:dict):
        super(QValueNet_SAC3, self).__init__()
        state_dim=int(param.get('w'))
        hidden_dim=int(param.get('hiden_dim'))
        action_dim=int(param.get('output'))
        self.fc1 = torch.nn.Linear(state_dim, hidden_dim)
