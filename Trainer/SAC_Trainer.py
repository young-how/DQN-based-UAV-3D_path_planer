...
class SAC_Trainer(BaseTrainer):
    ...
    def DFRL_sotf(self,model,update_rate):
        for param_target, param in zip(self.actor.parameters(),model.parameters()):
            param_target.data.copy_(param_target.data * (1.0 - update_rate) +param.data * update_rate)
    def Replace_Model(self,model):
        #self.actor=copy.copy(model)  #直接替换决策模型
        for param_target, param in zip(self.actor.parameters(),model.parameters()):
    ...
    def update(self,transition_dict):
        ...
        if self.IS_Continuous==1:
            ...
            actor_loss=torch.tensor([0], dtype=torch.float).to(self.device)
            #训练器参数为训练状态
            if self.Is_Train:
                # 更新两个Q网络
                td_target = self.calc_target(rewards, next_states, dones)
                ...
                min_qvalue = torch.sum(probs * torch.min(q1_value, q2_value),
                                    dim=1,
                                    keepdim=True)  # 直接根据概率计算期望
                actor_loss = torch.mean(-self.log_alpha.exp().to(self.device) * entropy - min_qvalue)
                #actor_loss = torch.mean(self.log_alpha.exp() * entropy - min_qvalue)
                self.actor_optimizer.zero_grad()
                actor_loss.backward()
                self.actor_optimizer.step()
                ...
                # 更新alpha值
                alpha_loss = torch.mean((entropy - self.target_entropy).detach().to(self.device) * self.log_alpha.exp().to(self.device))
                self.log_alpha_optimizer.zero_grad()
                alpha_loss.backward()
                self.log_alpha_optimizer.step()
    ...
    def get_policy(self,state):
        ...
        return  probs
    #返回策略分布(DFRL算法)
    def get_policy_DFRL(self,state):
        state = torch.tensor(state, dtype=torch.float).to(self.device)
        probs = self.actor(state)
        return  probs
